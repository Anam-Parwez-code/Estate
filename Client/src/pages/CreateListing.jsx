    ...
    <main className='bg-primary min-h-screen p-3 pb-10'>
      <div className='max-w-4xl mx-auto'>
        <h1 className='text-3xl font-bold text-center my-10 text-white'>
          Create a <span className='text-accent'>Listing</span>
        </h1>
          ...
          <div className='flex flex-col gap-4 flex-1'>
            ...
            <div className='flex flex-wrap gap-6 border-t border-slate-700 pt-6'>
              <div className='flex flex-col gap-1'>
                <label className='text-slate-400 text-xs ml-1'>Currency</label>
                <select 
                  id='currency' 
                  onChange={handleChange} 
              ...
              <div className='flex items-center gap-2'>
                ...
                <div className='flex flex-col items-start text-slate-300'>
                  ...
                  {formData.type === 'rent' && (
                    <span className='text-[10px] text-slate-500'>({formData.currency} / month)</span>
                  )}
                </div>
              </div>
              ...
              {formData.offer && (
                <div className='flex items-center gap-2'>
                  ...
                  <div className='flex flex-col items-start text-accent'>
                    ...
                    {formData.type === 'rent' && (
                      <span className='text-[10px] text-accent/70'>({formData.currency} / month)</span>
                    )}
                  </div>
                </div>
              )}
  ...
  <div className='flex flex-col gap-4'>
    ...
    >
      <svg className="w-10 h-10 text-slate-500 group-hover:text-accent mb-3 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="浸4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
      </svg>
      <p className='text-slate-400 group-hover:text-slate-200 text-sm'>
        Click to <span className='text-accent font-bold'>Upload</span> or drag and drop
      </p>
      <p className='text-slate-500 text-[10px] mt-1'>PNG, JPG up to 2MB</p>
      
      <input 
        onChange={(e) => setFiles(e.target.files)} 
        className='hidden' // Default input ko chhupa diya
        type='file' 
        id='images' 
        accept='image/*' 
        multiple 
    ...
    {/* Selected Files Count & Upload Button */}
