...
export default function ListingItem({ listing }) {
  ...
  const imageUrl =
    listing.imageUrls && listing.imageUrls.length > 0 ? listing.imageUrls[0] : FALLBACK_IMAGE;
    ...
    <div className='bg-slate-800/40 border border-slate-700/50 shadow-xl hover:shadow-accent/10 transition-all overflow-hidden rounded-2xl w-full sm:w-[330px] group'>
      <Link to={`/listing/${listing._id}`}>
        ...
        <div className='p-4 flex flex-col gap-3 w-full'>
          ...
          <p className='text-sm text-slate-500 line-clamp-2 italic'>{listing.description}</p>
