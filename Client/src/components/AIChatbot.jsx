...
export default function AIChatbot({ listings }) {
  ...
  // Sabse important: Aapka Render Backend URL
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      // Fix: 'response' key match (backend se yahi aa raha hai)
      const aiContent = response.data.response || response.data.reply || "I am sorry, I couldn't process that request.";
      ...
      console.log("Error or Timeout. Using Local Filter.");
      
      // Safety: Check if 'listings' is an array before filtering
      ...
      setMessages(prev => [...prev, { 
        ...
        content: foundListings.length > 0 
          ...
          : "I'm having trouble connecting to my brain, and I couldn't find a local match either!",
    ...
    <div className='fixed bottom-4 right-4 z-[1000]'>
      ...
      <div className={`${isOpen ? 'scale-100 opacity-100' : 'scale-0 opacity-0'} transition-all duration-300 origin-bottom-right absolute bottom-20 right-0 w-[320px] sm:w-[380px] h-[500px] bg-slate-900 border border-slate-700 rounded-3xl shadow-2xl flex flex-col overflow-hidden`}>
        ...
        <div className='flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-slate-900 to-slate-800 scrollbar-hide'>
            ...
            <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
              ...
              {m.results && m.results.map((listing) => (
                <div key={listing._id} className='mt-3 w-full bg-white rounded-xl overflow-hidden shadow-lg text-black'>
                  <img src={listing.imageUrls?.[0] || 'https://via.placeholder.com/300'} alt="" className='h-24 w-full object-cover' />
                  <div className='p-3 text-left'>
                    ...
                    <div className="flex justify-between items-center mt-2">
                       <span className='text-xs font-bold text-primary'>
                         {listing.currency || 'SAR'} {listing.regularPrice?.toLocaleString()}
                       </span>
          ...
          {isTyping && (
            <div className="flex items-start">
              <div className="bg-slate-700 p-4 rounded-2xl rounded-tl-none shadow-lg">
        ...
        <form onSubmit={handleSearch} className='p-4 bg-slate-800/80 border-t border-slate-700 flex gap-2'>
          ...
          />
          <button type='submit' disabled={isTyping} className='bg-accent text-primary p-3 rounded-xl hover:opacity-90 transition-all disabled:opacity-50'>
            <FaPaperPlane />
          </button>
        </form>
      </div>
